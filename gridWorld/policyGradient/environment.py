...
class Env(tk.Tk):
    ...
    def check_if_reward(self, state):
        ...
        for reward in self.rewards:
            if reward['state'] == state:
                ...
                # 타임스텝마다 -0.1의 보상을 받기때문에 수렴하는 값이 1이 아니다.
                if reward['reward'] > 0:
    ...
    def step(self, action):
        ...
        """
        입실론 탐욕 정책을 사용하지 않기 때문에 에이전트가 지속적인 탐험을 하기 어렵다.
        - 초반에 초록색세모(-1)에 많이 부딪힌다. 부딪히지 않도록 학습하게되고 이에따라 시작점에서 움직않게되는 문제가 발생
            L 타임스텝마다 -0.1의 보상을 주어 가만히 시작점에 머무는 행동이 좋은 행동이 아닌 것임을 알 수 있도록 한다. 
        """
        reward -= 0.1
